...
function createPaletteCorsHeaders(init) {
  const headers = new Headers(init);
  headers.set("Access-Control-Allow-Origin", "*");
  return headers;
}

function createPaletteJsonHeaders(status) {
  const headers = createPaletteCorsHeaders({
...
async function handlePalette(request) {
  ...
  if (request.method !== "GET") {
    return new Response(JSON.stringify({ error: "Method not allowed" }), {
      status: 405,
      headers: createPaletteJsonHeaders(405)
    });
  }
  ...
  if (!imageParam) {
    return new Response(JSON.stringify({ error: "Missing image parameter" }), {
      status: 400,
      headers: createPaletteJsonHeaders(400)
    });
  }
  ...
  if (!upstream.ok) {
    return new Response(JSON.stringify({ error: `Upstream request failed with status ${upstream.status}` }), {
      ...
      headers: createPaletteJsonHeaders(upstream.status)
    });
  }
  ...
  if (!contentType.startsWith("image/")) {
    return new Response(JSON.stringify({ error: "Unsupported content type" }), {
      status: 415,
      headers: createPaletteJsonHeaders(415)
    });
  }
  ...
  try {
    ...
    const response = new Response(JSON.stringify(palette), {
      status: 200,
      headers: createPaletteJsonHeaders(200)
    });
    ...
    return new Response(JSON.stringify({ error: "Failed to analyze image" }), {
      status: 500,
      headers: createPaletteJsonHeaders(500)
    });
  }
}
